	...
	for (std::string line; std::getline(std::cin, line);) {
		...
		try {
			...
			if (std::regex_search(line, match, reStart) && match.size() > 3) {
				...
				commandQueue.push(std::move(std::make_unique<CommandStart>(name, width, height)));
				...
				commandQueue.push(std::move(std::unique_ptr<DrawingCommand>{new CommandCircle(radius, centerX, centerY)}));
			} else if (std::regex_search(line, match, reExit)
					&& match.size() > 3) {
				commandQueue.push(std::move(std::unique_ptr<DrawingCommand>{new CommandExit()}));
